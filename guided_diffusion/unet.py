...
class CrossSliceAttention(nn.Module):
    ...
    def forward(self, center_slice, neighbor_slice):
        # Inputs have shape (batch, channels, height, width)
        b, c, h, w = center_slice.shape

        # Apply linear projections using 1x1 Conv2D
        q = self.to_q(center_slice)  # (b, inner_dim, h, w)
        k = self.to_k(neighbor_slice)  # (b, inner_dim, h, w)
        v = self.to_v(neighbor_slice)  # (b, inner_dim, h, w)

        # Reshape for multi-head attention
        q = q.view(b, self.heads, -1, h * w).permute(0, 1, 3, 2)  # (b, heads, tokens, dim_head)
        k = k.view(b, self.heads, -1, h * w).permute(0, 1, 3, 2)  # (b, heads, tokens, dim_head)
        v = v.view(b, self.heads, -1, h * w).permute(0, 1, 3, 2)  # (b, heads, tokens, dim_head)

        # Scaled dot-product attention
        dots = torch.matmul(q, k.transpose(-1, -2)) * self.scale  # (b, heads, tokens, tokens)
        attn = self.attend(dots)  # (b, heads, tokens, tokens)
        attn = self.dropout(attn)

        # Compute attention output
        out = torch.matmul(attn, v)  # (b, heads, tokens, dim_head)

        # Reshape back to original dimensions
        out = out.permute(0, 1, 3, 2).contiguous().view(b, -1, h, w)  # (b, inner_dim, h, w)
...
class InSliceAttention(nn.Module):
    ...
    def forward(self, center_slice):
        ...
        # Apply linear projections using 1x1 Conv2D
        q = self.to_q(center_slice)  # (b, inner_dim, h, w)
        k = self.to_k(center_slice)  # (b, inner_dim, h, w)
        v = self.to_v(center_slice)  # (b, inner_dim, h, w)
...
class UNetModel_MS_Former(nn.Module):
    ...
    def __init__(
        ...
        self.cross_slice_attention = CrossSliceAttention(dim=512)
        self.in_slice_attention = InSliceAttention(dim=512)

        # Projection layer to align attention output for the fusion block
        self.projection = nn.Conv2d(512*7, 512, kernel_size=1)
